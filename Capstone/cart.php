<?php
    ...
    //fetching all the items from cart to show on page
...
<html>
    <head>
        <title>Technoholic-Shopping Cart</title>
		<!-- Required meta tags -->
		<meta charset="utf-8">
        ...
        <script>
            $(document).ready(function(){
                //On change event listioner to update the quantity of product in the cart
                $(".productQuantity").change(function(){
                    var quantity = parseInt($(".productQuantity").val());
                    $.ajax({    
                        url: "rating.php",
                        type: "POST",
                        data: {act: "quantityChange", quantity: quantity, product_id: this.id, user_id: <?php echo $_SESSION["user_id"];?>},
                        success: function(data){
                            if(data == 0){
                                swal("Sorry!", "Valid input is required.", "error");
                            }
                        }
                    });
                });
                ...
                //click event listioner to remove an item from the cart
                        ...
                        buttons: ["Cancel", "Remove"]
                            ...
                            if(okay){
                                $.ajax({
                                    url: "rating.php",
                                    type: "POST",
                                    data: {act: "removeCartItem", product_id: this.id, user_id: <?php echo $_SESSION["user_id"];?>},
                                    success: function(data){
                                        //refresh the page
    ...
    <nav id="nav" class="navbar navbar-inverse navbar-fixed-top">
        ...
        <div class="container-fluid" id="main">
            <h1 style="text-align: center;"><span class='glyphicon glyphicon-shopping-cart'></span> Shopping-Cart</h1>
            <hr class="hr"/>
            <div  id="cart" >
                <?php
                //Show an empty message if there is nothing in the cart
                if(sizeof($items) == 0){
                    echo 'Your cart is empty at the moment.';
                }else{
                    //Loop through items in the cart and show them on page
            ...
            <a href="checkout.php"><button type='button' id="checkoutButton" class='btn btn-success'>Proceed to Buy</button></a>
        </div>
